...
def get_setting():
    ...
    if args.dataset == 'mnist':
        ...
        indices = get_index_of_classes(torch.tensor(target.targets), list(range(0, args.common_classes)))
        target = Subset(target, indices)

        indices = get_index_of_classes(torch.tensor(source.targets), list(range(0, 65)))
        source = Subset(source, indices)
